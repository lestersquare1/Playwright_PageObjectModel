...
test("Check Home Page", async ({ page }) => {
  ...
  await expect(homePage.purpose_heading).toHaveText(
    "Purpose-built for professionals"
  );
  await expect(homePage.purposeFirst_text).toHaveText(
    "PPS Group has been insuring professionals for " +
  ...
  await expect(homePage.purposeSecond_text).toHaveText(
    "With a proposition focused on protecting " +
      "professionals like you, you can feel confident knowing you’re sharing risk with like-minded individuals."
  );
  await expect(homePage.purposeThird_text).toHaveText(
    "We’re for professionals – like you."
  );

  await expect(homePage.mutuality_image).toBeVisible();
  await expect(homePage.mutuality_heading).toHaveText(
    "Mutuality is the best policy"
  );
  await expect(homePage.mutualityFirst_text).toHaveText(
    "PPS operates as a network of mutual insurers " +
  ...
  await expect(homePage.mutualitySecond_text).toHaveText(
    "Our members can take comfort that all PPS " +
  ...
  await expect(homePage.mutualityThird_text).toHaveText(
    "This is a model built on mutuality – where " +
      "real value is created together."
  );

  await expect(homePage.paysToBeMember_image).toBeVisible();
  await expect(homePage.paysToBeMember_heading).toHaveText(
    "It pays to be a member"
  );
  await expect(homePage.paysToBeMemberFirst_text).toHaveText(
    "Unlike most life insurers, PPS Mutual " +
  ...
  await expect(homePage.paysToBeMemberSecond_text).toHaveText(
    "Believe success is best when shared?  " + "The feeling is mutual."
  );

  await expect(homePage.whatIsMutuality_heading).toHaveText(
    "What is mutuality?"
  );
  await expect(homePage.whatIsMutuality_button).toHaveText("Watch video");
  await expect(homePage.ppsBakery_image).toBeVisible();
  ...
  await expect(homePage.footerInsuranceForProfessionals_link).toHaveText(
    "Insurance for professionals"
  );
  await expect(homePage.footerProfitShareBenefit_link).toHaveText(
    "Profit Share Benefit"
  );
  await expect(homePage.footerAboutUs_link).toHaveText("About us");
  await expect(homePage.footerClaims_link).toHaveText("Claims");
  await expect(homePage.footerRegulatoryAndConduct_link).toHaveText(
    "Regulatory and conduct"
  );
  await expect(homePage.footerContactUs_link).toHaveText("Contact us");
  await expect(homePage.footerFrequentlyAskedQuestions_link).toHaveText(
    "Frequently asked questions"
  );
  await expect(homePage.footerProfessionals_heading).toHaveText(
    "Professionals"
  );
  ...
  await expect(homePage.footerBecomeAnAdviser_link).toHaveText(
    "Become an adviser"
  );
